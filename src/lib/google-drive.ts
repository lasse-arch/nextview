...
/**
 * Finds the shared contracts folder (by name) among files this app has
 * created, or creates it if it doesn't exist yet. Scoped by drive.file, so
 * this only ever sees/creates files the app itself owns - never the
 * connected account's other Drive content.
 */
export async function findOrCreateContractsFolder(account: EmailAccount): Promise<string> {
  const accessToken = await getValidAccessToken(account);

  const query = `mimeType='application/vnd.google-apps.folder' and name='${CONTRACTS_FOLDER_NAME}' and trashed=false`;
  ...
  const createRes = await fetch(`${DRIVE_API_BASE}/files`, {
    ...
    body: JSON.stringify({ name: CONTRACTS_FOLDER_NAME, mimeType: "application/vnd.google-apps.folder" }),
  });
